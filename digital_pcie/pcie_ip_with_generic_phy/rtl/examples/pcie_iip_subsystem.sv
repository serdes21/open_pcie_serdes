...
wire  slv_aclk_active;
  ...
  // defined for internal use for EP
  wire    [3:0] device_type;
  assign device_type = `PCIE_EP;
...
// ======================================================================
//            PHY and clk_rst block instantiation
// ======================================================================
...
wire   [WIDTH_WD-1:0]           mac_phy_width;
...
localparam PHY_VPT_NUM = `CX_PHY_NUM_MACROS;      // Number of PHY xN sub-blocks
localparam PHY_VPT_DATA = `CX_PHY_VIEWPORT_DATA;  // Data width of PHY register interface
wire                                    phy_reg_clk_ug;
wire                                    phy_reg_clk_g;
wire                                    en_phy_reg_clk_g;
wire                                    phy_reg_rst_n;
wire    [PHY_VPT_NUM-1:0]               phy_cr_para_ack ;
wire    [PHY_VPT_NUM*PHY_VPT_DATA-1:0]  phy_cr_para_rdata;
wire    [PHY_VPT_NUM-1:0]               phy_cr_para_wr_en;
wire    [PHY_VPT_NUM-1:0]               phy_cr_para_rd_en;
wire    [15:0]                          phy_cr_para_addr;
wire    [PHY_VPT_DATA-1:0]              phy_cr_para_wr_data;

   assign phy_reg_clk_g       = '0;
   assign phy_cr_para_wr_en   = '0;
   assign phy_cr_para_rd_en   = '0;
   assign phy_cr_para_addr    = '0;
   assign phy_cr_para_wr_data = '0;




wire [NF-1 :0]                  cfg_pm_no_soft_rst;

wire                            core_clk_ug;
wire                            clkrst_sync_perst_n;
wire                            pwr_rst_n;
wire                            sticky_rst_n;
...
`ifndef SNPS_PCIE_SATB
  // include the PHY specific instance code
  `include "phy_instance.vh"


`endif //SNPS_PCIE_SATB

// --------------------------------------------------------
// CLK/RST block connection
// --------------------------------------------------------
...
`SNPS_PCIE_CLKRST_MODULE u_clk_rst (
    .mac_phy_txelecidle                 (mac_phy_txelecidle),
    .mac_phy_txcompliance               (mac_phy_txcompliance),

    // CPCIe specific signals
    .test_pclk                          (1'b1),
    .test_coreclk                       (1'b1),
    .test_auxclk                        (1'b1),
    .phy_status                         (phy_mac_phystatus),
    .mac_phy_rate                       (3'b00),
    ...
    .mstr_axi_clk                       (mstr_aclk),
    .mstr_axi_resetn                    (mstr_aresetn),
    .mstr_axi_clk_active                (mstr_aclk_active),
    .mstr_axi_clk_gated                 (mstr_aclk_gated),
    .slv_axi_clk                        (slv_aclk),
    .slv_axi_resetn                     (slv_aresetn),
    .slv_axi_clk_active                 (slv_aclk_active),
    .slv_axi_clk_gated                  (slv_aclk_gated),
// -------------------------------------------------------------------------------------
// CXS Clock & Reset
// -------------------------------------------------------------------------------------
    .pm_req_phy_rst                     (pm_req_phy_rst),
    .perst_n                            (perst_n),
    .sync_perst_n                       (clkrst_sync_perst_n),
    .power_up_rst_n                     (power_up_rst_n),
    .button_rst_n                       (button_rst_n),
    .pwr_rst_n                          (pwr_rst_n),
    .auxclk                             (pclk),
    .aux_clk_active                     (aux_clk_active),
    .en_muxd_aux_clk_g                  (en_muxd_aux_clk_g),
    .en_radm_clk_g                      (en_radm_clk_g),
    .link_req_rst_not                   (link_req_rst_not),
    .app_ltssm_enable                   (app_ltssm_enable),
    .wake_ref_rst_n                     (wake_ref_rst_n),
...
// -------------------------------------------------------------------------
// PHY testbench backdoor(s)
// -------------------------------------------------------------------------

  `ifndef UL_CDM
      `include "phy_tb_backdoor.svh"
  `endif // UL_CDM
